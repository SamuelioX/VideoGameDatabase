...
FROM video_game_info
    ...
    LEFT JOIN publisher on publisher.id = game_publisher.game_id
    LEFT JOIN genre_info on genre_info.id = game_genre.game_id
    GROUP BY video_game_info.id;*/
    /**
    /**CREATE TABLE tmp
SELECT game_id, developer_id FROM videogame.game_developer
	GROUP BY game_id, developer_id;

DROP TABLE game_developer;
ALTER Table tmp RENAME TO game_developer;*//**
SELECT 
	(SELECT id FROM video_game_info WHERE name = "3 Ninjas Kick Back") as game_id,
    (SELECT id FROM developer WHERE name = "Malibu Interactive") as developer_id,
    (SELECT id FROM publisher WHERE name = "Sony Imagesoft") as publisher_id; */
    /**SELECT video_game_info.*, group_concat(system_info.name) AS 'System Name',
system_info.id AS 'System ID', 
developer.name AS 'developer name',
publisher.name AS 'publisher name',
